...
func midi_params() -> Array:
	return [
		...
		{"id": "poultry_scale", "label": "Poultry cell size", "set": func(v):
			...
			s._poultry.push()},
		{"id": "poultry_adherence", "label": "Poultry adherence (the picture shows through)", "set": func(v):
			s._poultry.adherence = v
			s._poultry.push()},
		{"id": "poultry_fat", "label": "Poultry fat / skinny cells", "set": func(v):
			s._poultry.fat = v
			s._poultry.push()},
...
func midi_actions() -> Array:
	...
	out.append({"id": "jag_mode", "label": "Next trail shape (angular / branching / broken)", "do": func(): s.set_jag_mode(s.fb_jag_mode + 1)})
	...
	out.append({"id": "wake_toggle", "label": "Wake (sparks along the cursor) on / off", "do": func(): s.set_wake(not s.wake_on)})
	out.append({"id": "cues_stop", "label": "Stop the cue sheet", "do": s.stop_cues})
